...
export default function CalendarioManagement() {
  ...
  const fetchTemporadas = async () => {
    const url = mostrarInactivas ? '/api/admin/temporadas?incluir_inactivas=true' : '/api/admin/temporadas';
  ...
  const handleSincronizarCalendario = async (temporadaId: string, temporadaNombre: string) => {
    if (!confirm(`¿Sincronizar el calendario de "${temporadaNombre}" con la interfaz de anotadores?\n\nEsto recreará los partidos disponibles para los anotadores basándose en el calendario existente.`)) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/admin/sincronizar-calendario/${temporadaId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' }
      });
      ...
      if (response.ok) {
        setSuccess(`Calendario sincronizado: ${data.estadisticas.partidosCreados} partidos disponibles para anotadores`);
        fetchTemporadas();
      } else {
        setError(data.error || 'Error sincronizando calendario');
      }
    } catch (error) {
      setError('Error de conexión');
    } finally {
  ...
  const handleEliminarTemporada = async (temporadaId: string, temporadaNombre: string) => {
    if (!confirm(`¿Estás seguro de eliminar la temporada "${temporadaNombre}"?\n\nEsta acción no se puede deshacer y eliminará:\n- La temporada\n- Todas las jornadas asociadas\n\nNota: Solo se pueden eliminar temporadas inactivas`)) {
      return;
    }
    ...
    try {
      const response = await fetch(`/api/admin/temporadas?id=${temporadaId}`, {
        method: 'DELETE',
      });
      ...
      if (response.ok) {
        ...
        setError(data.error || 'Error eliminando temporada');
      }
    } catch (error) {
      setError('Error de conexión');
    } finally {
  ...
  const handleToggleActivoTemporada = async (temporadaId: string, temporadaNombre: string, activoActual: boolean) => {
    setSaving(true);
    setError('');
    setSuccess('');
    ...
    try {
      // Intentar con el endpoint simple primero
      let response = await fetch(`/api/admin/temporadas/toggle-simple?id=${temporadaId}&activo=${nuevoEstado}`, {
        method: 'PATCH',
      });
      ...
      // Si falla por problemas de esquema, intentar con el endpoint alternativo
      if (data.error && (data.error.includes('schema cache') || data.error.includes('activo'))) {
        console.log('🔄 Intentando con método alternativo...');
        
        response = await fetch(`/api/admin/temporadas/toggle-fallback?id=${temporadaId}&accion=${accion}`, {
          method: 'PATCH',
        });
        ...
        if (response.ok) {
          if (data.fallback_mode) {
            setSuccess(`${data.message} ⚠️ Modo alternativo - ${data.note}`);
          } else {
            setSuccess(data.message);
          }
          fetchTemporadas();
      ...
      // Si ambos fallan, mostrar error con instrucciones
      if (data.needsSetup || data.needsRestart) {
        setError(`${data.error} Usa el botón "🔧 Configurar Sistema" para obtener instrucciones completas.`);
      } else {
        setError(data.error || 'Error cambiando estado de temporada');
      }
    } catch (error) {
      setError('Error de conexión');
    } finally {
  ...
  const handleSetupActivoField = async () => {
    setSaving(true);
    setError('');
    setSuccess('');

    try {
      const response = await fetch('/api/admin/temporadas/setup-activo', {
        method: 'POST',
      });
      ...
      if (response.ok) {
        // Mostrar las instrucciones SQL
        const sqlCommands = data.sql_instructions.join('\n\n');
...
📝 Pasos COMPLETOS:
${data.step_by_step.join('\n')}`;

        if (confirm(message + '\n\n¿Copiar SQL al portapapeles?')) {
          navigator.clipboard.writeText(sqlCommands).then(() => {
            setSuccess('✅ SQL copiado al portapapeles. Ve a tu dashboard de Supabase y ejecuta el SQL.');
          }).catch(() => {
            setSuccess('✅ Instrucciones mostradas. Copia manualmente el SQL de arriba.');
          });
        } else {
          setSuccess('📋 Instrucciones de configuración mostradas en el diálogo anterior.');
        }
      } else {
        setError(data.error || 'Error obteniendo instrucciones de configuración');
      }
    } catch (error) {
      setError('Error de conexión');
    } finally {
  ...
  if (session?.user?.role !== 'admin') {
    return (
      <div className="text-center p-8">
        ...
        <p className="text-gray-900 mt-2">Solo los administradores pueden gestionar calendarios.</p>
      </div>
    );
  }
        ...
        {error && (
          <div className="mb-6 bg-red-500/20 border border-red-500/50 text-red-300 px-6 py-4 rounded-xl backdrop-blur-sm">
            <div className="flex items-center justify-between">
              ...
              {(error.includes('temporada no encontrada') || error.includes('activo') || error.includes('does not exist') || error.includes('schema cache') || error.includes('Configurar Sistema')) && (
                <button
                  onClick={handleSetupActivoField}
                  disabled={saving}
                  className="bg-blue-600/20 text-blue-300 border border-blue-500/50 hover:bg-blue-600/30 px-3 py-1 rounded text-sm font-medium transition-all duration-200"
                >
                  🔧 Configurar Sistema
                </button>
              )}
                ...
                <div className="flex justify-between items-center mb-6">
                  ...
                  <div className="flex items-center space-x-4">
                    <label className="flex items-center text-white">
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setTimeout(() => fetchTemporadas(), 100);
                        }}
                    ...
                    <button
                      onClick={handleSetupActivoField}
                      disabled={saving}
                      className="bg-gradient-to-r from-purple-600 via-purple-700 to-purple-800 hover:from-purple-700 hover:via-purple-800 hover:to-purple-900 text-white px-4 py-2 rounded-xl font-semibold transition-all duration-300 transform hover:scale-105 shadow-lg text-sm"
                      title="Configurar sistema de activar/desactivar temporadas"
                    >
                      🔧 Configurar Sistema
                    </button>
                    <button
                ...
                {temporadas.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {temporadas.map((temporada) => (
                      <div key={temporada.id} className={`border border-slate-600 rounded-xl p-6 hover:bg-slate-700/70 transition-all duration-300 ${
                        (temporada.activo ?? true) ? 'bg-slate-700/50' : 'bg-slate-800/70 opacity-75'
                      }`}>
                        <div className="flex items-center justify-between mb-4">
                          <div className="flex items-center space-x-3">
                            <h4 className="font-semibold text-white text-lg">{temporada.nombre}</h4>
                            {!(temporada.activo ?? true) && (
                              <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-gray-500/20 text-gray-300 border border-gray-500/50">
                                Inactiva
                              </span>
                            )}
                          </div>
                        ...
                        <div className="mt-6 flex justify-between items-center">
                          <div className="flex space-x-2">
                            ...
                            <button
                              onClick={() => handleToggleActivoTemporada(temporada.id, temporada.nombre, temporada.activo ?? true)}
                              disabled={saving}
                              className={`px-3 py-1 rounded text-sm font-medium transition-all duration-200 ${
                                (temporada.activo ?? true)
                                  ? 'bg-orange-600/20 text-orange-300 border border-orange-500/50 hover:bg-orange-600/30'
                                  : 'bg-green-600/20 text-green-300 border border-green-500/50 hover:bg-green-600/30'
                              }`}
                            >
                              {(temporada.activo ?? true) ? '⏸️ Desactivar' : '▶️ Activar'}
                            </button>
                          </div>
                          <div className="flex space-x-2">
                            {temporada.estado === 'configuracion' && (temporada.activo ?? true) && (
                              <button
                            ...
                            {temporada.estado === 'generado' && (temporada.activo ?? true) && (
                              <button
                                onClick={() => handleSincronizarCalendario(temporada.id, temporada.nombre)}
                                disabled={saving}
                                className="bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-all duration-300 transform hover:scale-105 disabled:opacity-50 disabled:transform-none"
                                title="Sincronizar calendario con interfaz de anotadores"
                              >
                                {saving ? '⏳' : '🔄'} Sincronizar
                              </button>
                            )}
                            {!(temporada.activo ?? true) && (
                              <button
                                onClick={() => handleEliminarTemporada(temporada.id, temporada.nombre)}
                                disabled={saving}
                                className="bg-red-600/20 text-red-300 border border-red-500/50 hover:bg-red-600/30 px-3 py-1 rounded text-sm font-medium transition-all duration-200"
                                title="Eliminar temporada inactiva"
                              >
                                {saving ? '⏳' : '🗑️'} Eliminar
                              </button>
                            )}
                            ...
                            {temporada.estado === 'configuracion' && (
                              <>
                                ...
                                <button
                                  ...
                                  title="Eliminar temporada (solo disponible para temporadas en configuración)"
                                >
